...
def gen_qst(type="HTS"):
    ...
    # 3-phone
    #questionset.append(get_QS(QS_phone,"L-",symbol_phone[0],symbol_phone[1]))
    #questionset.append(get_QS(QS_phone,"C-",symbol_phone[1],symbol_phone[2]))
    #questionset.append(get_QS(QS_phone,"R-",symbol_phone[2],symbol_phone[3]))
...
if __name__ == '__main__':
    ...
    for qst in aditional_qst :
        qst_DNN.append(qst)

    qes_dir_path = os.path.join(os.getcwd(), "output_questionset")
    ...
    with open("output_questionset/questions_qst001.hed",'w') as fout:
        for items in qst_HTS:
            for item in items:
                fout.write(item)

    with open("output_questionset/questions_utt_qst001.hed",'w') as fout:
        for items in qst_utt_HTS:
            for item in items:
                fout.write(item)

    with open("output_questionset/questions_qst001.conf",'w') as fout:
        for items in qst_DNN:
            for item in items:
                fout.write(item)

    #symbol_phone=['/A:','^','-','+','+','/B:']
    qst_config_path = os.path.join("output_questionset","qstConfigs.json")
    ...
    print("QS_phone",len(QS_phone))
    print("QS_syl_vowel",len(QS_syl_vowel))
    print("qst_DNN",len(qst_DNN))
    '''
    print(qst_configs)
    qst_list = list(qst_configs.keys())

    string = " 2995000  3932500 /A:y^uw-w+er+m/B:"
    pattern = qst_configs[qst_list[1]]
