...
def list_shift(listP , direction , count , _type_):
    ...
    if direction == "previous" and _type_ == "int" :
        for i in range(0,count):
            listP.pop(-1)
            listP.insert(0 , 0 )
    elif direction == "previous" and _type_ == "char" :
        for i in range(0,count):
            listP.pop(-1)
            listP.insert(0 , '0')
    elif direction == "next" and _type_ == "int":
        for i in range(0,count):
            listP.pop(0)
            listP.insert(len(listP) , 0 )
    elif direction == "next" and _type_ == "char":
        for i in range(0,count):
            listP.pop(0)
            listP.insert(len(listP) , '0' )
    else:
...
class PHONE_LABEL():
    ...
    def Get_label(self,text,last=False):
        ...
        self.LL_phone = list_shift(self.C_phone.copy() , "previous" , 2 , 'char')
        self.L_phone = list_shift(self.C_phone.copy() , "previous" , 1 , 'char')
        self.R_phone = list_shift(self.C_phone.copy() , "next" , 1 , 'char')
        self.RR_phone = list_shift(self.C_phone.copy() , "next" , 2 , 'char')
...
class SYLLABLE_LABEL():
    ...
    def Get_label(self,text,phone_start,syllable_start,phone_num_in_syllable,stress_count):
        ...
        phone_num_in_previous_syllable = list_shift(phone_num_in_syllable.copy(),"previous", 1 , "int")
        stress_count_in_previous_syllable = list_shift(stress_count.copy(),"previous", 1 , "char")
        phone_num_in_current_syllable = phone_num_in_syllable.copy()
        stress_count_in_current_syllable = stress_count.copy()
        phone_num_in_next_syllable = list_shift(phone_num_in_syllable.copy(),"next", 1 , "int")
        stress_count_in_next_syllable = list_shift(stress_count.copy(),"next", 1 , "char" )
        self.ph_in_presyl = Generate_single_label(text,phone_num_in_previous_syllable,syllable_start,[],[],[], "int")
        self.ph_in_cursyl = Generate_single_label(text,phone_num_in_current_syllable,syllable_start,[],[],[], "int")
        self.ph_in_nextsyl = Generate_single_label(text,phone_num_in_next_syllable,syllable_start,[],[],[], "int")
        self.vowl_cursyl = Generate_single_label(text,syl_vowel,syllable_start,[],[],[], "int")
...
class WORD_LABEL():
    ...
    def Get_label(self,text,word_start,syllable_num_in_word):
        syllable_num_in_previous_word = list_shift(syllable_num_in_word.copy(),"previous", 1 , "int" )
        syllable_num_in_current_word = syllable_num_in_word.copy()
        syllable_num_in_next_word = list_shift(syllable_num_in_word.copy(),"next", 1 , "int" )
        self.syl_in_preword = Generate_single_label(text, syllable_num_in_previous_word,[] ,word_start, [] ,[], "int")
        self.syl_in_curword = Generate_single_label(text, syllable_num_in_current_word,[] , word_start, [],[], "int")
        self.syl_in_nextword = Generate_single_label(text, syllable_num_in_next_word, [] ,word_start, [] ,[], "int")
...
class sentence_LABEL():
    ...
    def Get_label(self,text,sentence_start,syllable_num_in_sentence,word_num_in_sentence):
        syllable_num_in_previous_sentence = list_shift(syllable_num_in_sentence.copy(),"previous",1,"int")
        word_num_in_previous_sentence = list_shift(word_num_in_sentence.copy() , "previous",1,"int" )
        syllable_num_in_current_sentence = syllable_num_in_sentence.copy()
        word_num_in_current_sentence = word_num_in_sentence.copy()
        syllable_num_in_next_sentence = list_shift(syllable_num_in_sentence.copy(),"next",1,"int")
        word_num_in_next_sentence = list_shift(word_num_in_sentence.copy() , "next",1,"int" )


        self.syl_in_presentence = Generate_single_label(text, syllable_num_in_previous_sentence, [] ,[], sentence_start,[], "int")
        self.word_in_presentence = Generate_single_label(text, word_num_in_previous_sentence, [] ,[], sentence_start ,[], "int")
        self.syl_in_cursentence = Generate_single_label(text, syllable_num_in_current_sentence, [] ,[], sentence_start,[], "int")
        self.word_in_cursentence = Generate_single_label(text, word_num_in_current_sentence, [] ,[], sentence_start ,[], "int")
        self.syl_in_nextsentence = Generate_single_label(text, syllable_num_in_next_sentence,[] ,[], sentence_start,[], "int")
        self.word_in_nextsentence = Generate_single_label(text, word_num_in_next_sentence, [] ,[], sentence_start ,[], "int")
...
class utterance_LABEL():
    ...
    def Get_label(self,text,utterance_start,syllable_num_in_utterance,word_num_in_utterance,sentence_num_in_utterance):
        self.syl_in_utterance = Generate_single_label(text, syllable_num_in_utterance,[] ,[], [],utterance_start, "int")
        self.word_in_utterance = Generate_single_label(text, word_num_in_utterance,[] ,[], [] ,utterance_start, "int")
        self.sentence_in_utterance = Generate_single_label(text, sentence_num_in_utterance,[] ,[], [],utterance_start, "int")
...
class POS_LABEL():
    ...
    def Get_label(self,text,word_start,GPOS_list):
        GPOS_list_in_previous_word = list_shift(GPOS_list.copy(),"previous", 1 , "char")
        GPOS_list_in_current_word = GPOS_list.copy()
        GPOS_list_in_next_word = list_shift(GPOS_list.copy(),"next", 1 , "char")
        self.pos_preword = Generate_single_label(text, GPOS_list_in_previous_word,[] ,word_start, [] , [],"char")
        self.pos_curword = Generate_single_label(text, GPOS_list_in_current_word , [], word_start, [],[], "char")
        self.pos_nextword = Generate_single_label(text, GPOS_list_in_next_word,[] ,word_start, [] ,[], "char")
...
class TONE_LABEL():
    ...
    def Get_label(self,text,syllable_start,word_start,sentence_start,stress_count,syllable_num_in_sentence,word_num_in_sentence,stress_tag):
        stress_count_shift_right = list_shift(stress_count.copy(),"previous", 1 , "int" )
        current_syllable_num_in_current_sentence = syllable_num_in_sentence.copy()
        word_num_in_current_sentence = word_num_in_sentence.copy()
        stress_tag_in_previous_syllable = list_shift(stress_tag.copy(),"previous", 1 , "char")
        stress_tag_in_current_syllable = stress_tag.copy()
        stress_tag_in_next_syllable = list_shift(stress_tag.copy(),"next", 1 , "char" )
        ...
        content_index_shift_right = list_shift(content_index.copy(),"previous", 1 , "char" )
        ...
        self.stress_presyl = Generate_single_label(text,stress_tag_in_previous_syllable,syllable_start,[],[],[], "int")
        self.stress_cursyl = Generate_single_label(text,stress_tag_in_current_syllable,syllable_start,[],[],[], "int")
        self.stress_nextsyl = Generate_single_label(text,stress_tag_in_next_syllable,syllable_start,[],[],[], "int")
...
class Labelforall():
    ...
    def Get_label(self,text,\
            phone_start,syllable_start,word_start,sentence_start,utterance_start, \
        ...
        self.LLphone = list_shift(self.Cphone.copy() , "previous" , 2 , 'char')
        self.Lphone = list_shift(self.Cphone.copy() , "previous" , 1 , 'char')
        self.Rphone = list_shift(self.Cphone.copy() , "next" , 1 , 'char')
        self.RRphone = list_shift(self.Cphone.copy() , "next" , 2 , 'char')
        ...
        self.vowel = Generate_single_label(text,syl_vowel,syllable_start,[],[],[], "int")

        # stress
        stress_tag_in_previous_syllable = list_shift(stress_tag.copy(),"previous", 1 , "char")
        stress_tag_in_current_syllable = stress_tag.copy()
        stress_tag_in_next_syllable = list_shift(stress_tag.copy(),"next", 1 , "char" )
        self.Lstress = Generate_single_label(text,stress_tag_in_previous_syllable,syllable_start,[],[],[], "int")
        self.Cstress = Generate_single_label(text,stress_tag_in_current_syllable,syllable_start,[],[],[], "int")
        self.Rstress = Generate_single_label(text,stress_tag_in_next_syllable,syllable_start,[],[],[], "int")

        # Pos
        GPOS_list_in_previous_word = list_shift(GPOS_list.copy(),"previous", 1 , "char")
        GPOS_list_in_current_word = GPOS_list.copy()
        GPOS_list_in_next_word = list_shift(GPOS_list.copy(),"next", 1 , "char")
        self.Lpos = Generate_single_label(text, GPOS_list_in_previous_word,[] ,word_start, [] , [],"char")
        self.Cpos = Generate_single_label(text, GPOS_list_in_current_word , [], word_start, [],[], "char")
        self.Rpos = Generate_single_label(text, GPOS_list_in_next_word,[] ,word_start, [] ,[], "char")
        ...
        self.phones_utterance = Generate_single_label(text, phones_utterance,[] ,[], [],utterance_start, "int")
        self.syllables_utterance = Generate_single_label(text, syllables_utterance,[] ,[], [],utterance_start, "int")
        self.words_utterance = Generate_single_label(text, words_utterance,[] ,[], [] ,utterance_start, "int")
        #self.sentence_in_utterance = Generate_single_label(text, sentences_utterance,_ ,_, _,utterance_start, "int")

        self.phones_syllable = Generate_single_label(text,phones_syllable,syllable_start,[],[],[], "int")
        self.syllables_word = Generate_single_label(text, syllables_word,[] , word_start, [],[], "int")
        self.words_sentence = Generate_single_label(text, words_sentence, [] ,[], sentence_start ,[], "int")
        self.sentences_utterance = Generate_single_label(text, sentences_utterance,[] ,[], [],utterance_start, "int")
...
def write_label_file(filename,hts_label_dir_path,label,curphone,Start,End):
    ...
    with open(filepath1 , "w") as f :
        for lab in label:
            f.write(lab)
            f.write("\n")
        print("gen_htslabel_output :",filepath1)
    ...
    with open(filepath2 , "w") as f :
        for i , cphone in enumerate(curphone):
            ...
            f.write(string)
            f.write("\n")
        print("gen_htslabel_output :",filepath2)
...
def HtsLabel_Generation(mulIn_dir_path, textIn_path, htslabelOut_dir_path):
    ...
    for textInfo in textInfos:
        ...
        write_label_file(filename,htslabelOut_dir_path,label,curphone,Start,End)
