...
class SENTENSE_LABEL():
    ...
    def Get_label(self,text,sentence_start,syllable_num_in_sentence,word_num_in_sentence,phrase_num_in_sentence):
        ...
        # print("self.phrase_in_sentense :",self.phrase_in_sentense)
        for i in range(len(text)):
...
class POSITION_LABEL():
    ...
    def Get_label(self,text,phone_start,syllable_start,word_start,phrase_start, \
        ...
        self.b5,self.b6 = Generate_position_label(text,syllable_num_in_phrase,syllable_start,phrase_start)
        self.e3,self.e4 = Generate_position_label(text,word_num_in_current_phrase,word_start,phrase_start)
        self.h3,self.h4 = Generate_position_label(text,phrase_num_in_sentence,phrase_start,sentence_start)
        for i in range(len(text)):
...
class TONE_LABEL():
    ...
    def Get_label(self,text,syllable_start,word_start,phrase_start,stress_count,syllable_num_in_phrase,word_num_in_phrase,stress_tag):
        ...
        self.b7 = xxx_num_before_curent_xxx_in_phrase(text,stress_count_shift_right,syllable_start,phrase_start)
        self.b8 = xxx_num_after_curent_xxx_in_phrase(text,stress_count,stressed_num_in_phrase,syllable_start,phrase_start)
        self.b9, self.b10 = xxx_num_from_previous_xxx_to_current_xxx(text,stress_count,current_syllable_sum_in_current_phrase,syllable_start,phrase_start)
...
def gen_label(Start,End,phone,syllable,word,phrase,sentense,pos,tone,position):
...
def write_label_file(filename,hts_label_dir_path,label,curphone):
    ...
    filepath1 = os.path.join(hts_label_dir_path, "full" , filename.replace(".mul",".lab"))
    filepath2 = os.path.join(hts_label_dir_path, "mono" , filename.replace(".mul",".lab"))
...
phone_num_in_syllable,phone_num_in_word,phone_num_in_phrase, \
syllable_num_in_word,syllable_num_in_phrase,syllable_num_in_sentence, \
word_num_in_phrase,word_num_in_sentence, \
phrase_num_in_sentence):
    ...
    if max(syllable_num_in_phrase) > 20 : # 20 -> 59
        syllable_num_in_phrase = clip(syllable_num_in_phrase, 20)
    if max(syllable_num_in_sentence) > 28 : # 28-> 59
        syllable_num_in_sentence = clip(syllable_num_in_sentence, 28)
    if max(word_num_in_phrase) > 13 : # 13 -> 40
        word_num_in_phrase = clip(word_num_in_phrase, 13)
    if max(word_num_in_sentence) > 13 : # 13 -> 40
        word_num_in_sentence = clip(word_num_in_sentence, 13)
    if max(phrase_num_in_sentence) > 4 : # 4 -> 10
        phrase_num_in_sentence = clip(phrase_num_in_sentence, 4)
    ...
    if max(syllable_num_in_phrase) > 20 : # 20 -> 59
        print("syllable_num_in_phrase error :",max(syllable_num_in_phrase),filename)
    if max(syllable_num_in_sentence) > 28 : # 28-> 59
        print("syllable_num_in_sentence error :",max(syllable_num_in_sentence),filename)
    if max(word_num_in_phrase) > 13 : # 13 -> 40
        print("word_num_in_phrase error :",max(word_num_in_phrase),filename)
    if max(word_num_in_sentence) > 13 : # 13 -> 40
        print("word_num_in_sentence error :",max(word_num_in_sentence),filename)
    if max(phrase_num_in_sentence) > 4 : # 4 -> 10
        print("phrase_num_in_sentence error :",max(phrase_num_in_sentence),filename)

    return phone_num_in_syllable,syllable_num_in_word,syllable_num_in_phrase, \
        syllable_num_in_sentence,word_num_in_phrase,word_num_in_sentence, \
        phrase_num_in_sentence
...
if __name__ == '__main__':
    ...
    if args.dataset == "arctic":
        dataset_dir_path = "output_mul/arctic/"
        pattern_name = r'[ab]\d{4}'
        hts_label_dir_path = os.path.join(os.getcwd(), "output_htslabel", "arctic")
    elif args.dataset == "ljspeech":
        dataset_dir_path = "output_mul/ljspeech/"
        pattern_name = r'LJ\d+\-\d+'
        hts_label_dir_path = os.path.join(os.getcwd(), "output_htslabel", "ljspeech")

    #if not os.path.isdir(hts_label_dir_path):
	    #os.makedirs(hts_label_dir_path)

    for dirPath, dirNames, fileNames in os.walk(dataset_dir_path):
        mul_fileName = sorted(fileNames)
    #arctic_fileName = sorted(arctic_fileName[0:-1])

    #mul_fileName = mul_fileName[22:23]

    for filename in mul_fileName:
        ...
        # print("word_list :",word_list,len(word_list))
        # print("phrase_list :",phrase_list,len(phrase_list))
        # print("stress_count :",stress_count,len(stress_count))
        # print("stress_tag :",stress_tag,len(stress_tag))
        ...
        syllable = SYLLABLE.Get_label(text,phone_start,syllable_start,phone_num_in_syllable,stress_count)
        ...
        tone = TONE.Get_label(text,syllable_start,word_start,phrase_start,stress_count,syllable_num_in_phrase,word_num_in_phrase,stress_tag)
        #stress = STRESS.Get_label(text,stress_tag)
        position = POSITION.Get_label(text,phone_start,syllable_start,word_start,phrase_start, \
            phone_num_in_syllable,syllable_num_in_word,syllable_num_in_phrase,word_num_in_phrase,phrase_num_in_sentence)
        #tone = TONE.Get_label(text,syllable_start,word_start,phrase_start,stress_count,syllable_num_in_phrase,word_num_in_phrase)

        curphone = PHONE.Get_phone()
        #print(len(phone),len(phone3))
        label = gen_label(Start,End,phone,syllable,word,phrase,sentense,pos,tone,position)
        write_label_file(filename,hts_label_dir_path,label,curphone)
