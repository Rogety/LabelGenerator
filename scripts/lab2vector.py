...
if __name__ == '__main__':
    ...
    if args.dataset == "arctic":
        dataset_dir_path = "output_htslabel/arctic/full/"
        pattern_name = "arctic"
        vec_dir_path = os.path.join(os.getcwd(), "output_vector", "arctic")
        h5_dir_path = os.path.join(os.getcwd(), "output_hdf5", "wavenet","arctic")
    elif args.dataset == "ljspeech":
        dataset_dir_path = "output_htslabel/ljspeech/full/"
        pattern_name = "ljspeech"
        vec_dir_path = os.path.join(os.getcwd(), "output_vector", "ljspeech")
        h5_dir_path = os.path.join(os.getcwd(), "output_hdf5", "wavenet","ljspeech")

    if not os.path.isdir(vec_dir_path):
    ...
    # htslab_fileName = htslab_fileName[22:23]
    #arctic_fileName = arctic_fileName[:1]
    ...
    for index, filename in enumerate(htslab_fileName):
        vec_all = []
        durations = []
        filepath = dataset_dir_path + filename
        h5_filepath = os.path.join(h5_dir_path, filename.replace(".lab",".h5"))
        print("filename :",filename)
        print("filepath :",filepath)
        filename_idx = filename.replace(".lab","")
        err = 0
        ...
        #print(len(vec_all),len(vec_all[0]))
        #print(len(durations),len(durations[0]))
        #vec_all = list(itertools.chain.from_iterable(vec_all))
        vec_all = np.array(vec_all)
        durations = np.array(durations)
        print("linguisitic vector :",vec_all.shape)
        print("durations :",durations.shape)
        # print("vec_all",vec_all.shape)
        #print(len(result_vec))
        #vec_filename = '%s-lab-%05d.npy' % (pattern_name,(index+1))
        vec_filename = 'label-{}.npy'.format(filename_idx)
        vec_filepath = os.path.join(vec_dir_path , vec_filename)
        print("vec_filename :",vec_filename)
        np.save(vec_filepath,vec_all) # (phone, 762)

        with h5py.File(h5_filepath , 'w') as f :
            f['world'] = vec_all
            f['duration'] = durations

        print(vec_filename,"success")
        
        ## test h5 file
        '''
        feat1 = read_hdf5(h5_filepath, "/" + "world")
        feat2 = read_hdf5(h5_filepath, "/" + "duration")
        print(feat1.shape)
        print(feat1[0])
        print(feat2.shape)
        print(feat2)
